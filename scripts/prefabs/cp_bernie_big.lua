...
local assets =
{
    Asset("ANIM", "anim/cp_bernie_big.zip"),
    Asset("ANIM", "anim/cp_bernie_build.zip"),
    Asset("ANIM", "anim/cp_bernie_lunar_build.zip"),
    Asset("ANIM", "anim/cp_bernie_shadow_build.zip"),    
    Asset("ANIM", "anim/cp_bernie_lunar_fx.zip"), 
    Asset("ANIM", "anim/cp_bernie_shadow_fx.zip"),       
    Asset("ANIM", "anim/cp_bernie_fire_fx_shadow_build.zip"),
    Asset("ANIM", "anim/cp_bernie_fire_fx_lunar_build.zip"), 
...
local fireassets =
{
    Asset("ANIM", "anim/cp_bernie_fire_fx.zip"),
}
...
local function OnAttacked(inst, data)
    -- 获取攻击者，data 不为空时获取 data.attacker，否则为 nil
    local attacker = data ~= nil and data.attacker or nil
    
    -- 检查攻击者是否存在，并且如果防止目标被攻击函数返回 false，则执行以下代码
    if attacker ~= nil and not PreventTargetingOnAttacked(inst, attacker, TheNet:GetPVPEnabled() and "bernieowner" or "player") then
        ...
        -- 如果当前没有有效的攻击目标或者目标不在攻击范围内，则设置新的攻击目标为攻击者
...
local function OnLighterLight(inst)
    ...
    if inst.fire_thorns_task then
        ...
        -- 如果伯尼的主人激活了影之效忠技能树，则设置火焰特效的动画和颜色
        ...
        -- 如果伯尼的主人激活了月之效忠技能树，则设置火焰特效的动画和颜色
        ...
        -- 设置伤害反射的默认伤害值
        inst.components.damagereflect:SetDefaultDamage(TUNING.BERNIE_BURNING_REFLECT_DAMAGE * 40)
        -- 监听反射伤害事件
    ...
    -- 创建一个任务，在20秒后结束火焰荆棘效果
    inst.fire_thorns_task = inst:DoTaskInTime(20*40, function()
        endthornsfire(inst)
    end)
end
...
local function onLeaderChanged(inst,leader)    
    ...
    if leader and leader.components.skilltreeupdater:IsActivated("willow_bernieregen_2") then
        inst.components.health:StartRegen(TUNING.SKILLS.WILLOW_BERNIE_HEALTH_REGEN_2*40, TUNING.SKILLS.WILLOW_BERNIE_HEALTH_REGEN_PERIOD)
    elseif leader and leader.components.skilltreeupdater:IsActivated("willow_bernieregen_1") then
        inst.components.health:StartRegen(TUNING.SKILLS.WILLOW_BERNIE_HEALTH_REGEN_1*40, TUNING.SKILLS.WILLOW_BERNIE_HEALTH_REGEN_PERIOD)        
    end        
    ...
    if leader and leader.components.skilltreeupdater:IsActivated("willow_berniehealth_2") then 
        inst.components.health:SetMaxHealth(TUNING.BERNIE_BIG_HEALTH*40 * TUNING.SKILLS.WILLOW_BERNIEHEALTH_2)
    elseif leader and leader.components.skilltreeupdater:IsActivated("willow_berniehealth_1") then
        inst.components.health:SetMaxHealth(TUNING.BERNIE_BIG_HEALTH*40 * TUNING.SKILLS.WILLOW_BERNIEHEALTH_1)
    else
        inst.components.health:SetMaxHealth(TUNING.BERNIE_BIG_HEALTH*40)
    end 
...
local function CheckForAllegiances(inst, leader)
    ...
    if allegiance == 0 and (shadow or lunar) then -- and (inst:HasTag("shadow_aligned") or inst:HasTag("lunar_aligned")) 
        ...
        inst.components.planardamage:SetBaseDamage(TUNING.BERNIE_PLANAR_DAMAGE*40)
        inst.components.planardefense:SetBaseDefense(TUNING.BERNIE_PLANAR_DEFENCE*40)
    end
end
...
local function fn()
    ...
    -- 设置默认伤害值为 BERNIE_BIG_DAMAGE 的值
    inst.components.combat:SetDefaultDamage(TUNING.BERNIE_BIG_DAMAGE*40)
    -- 设置攻击间隔为 BERNIE_BIG_ATTACK_PERIOD 的值
    inst.components.combat:SetAttackPeriod(TUNING.BERNIE_BIG_ATTACK_PERIOD/40)
    -- 设置攻击距离为 BERNIE_BIG_ATTACK_RANGE 和 BERNIE_BIG_HIT_RANGE 的值
    inst.components.combat:SetRange(TUNING.BERNIE_BIG_ATTACK_RANGE*40, TUNING.BERNIE_BIG_HIT_RANGE*40)
    -- 设置重新选择目标的函数，每秒检查一次
    ...
    -- 设置战斗口号间隔为 16 秒
    inst.components.combat.battlecryinterval = 16/40
    -- 设置攻击命中效果显示的部位为身体
    ...
    -- 添加伤害类型抗性组件，用于管理对不同伤害类型的抗性
    inst:AddComponent("damagetyperesist")
    -- 添加伤害类型加成组件，用于管理对不同伤害类型的额外伤害
    ...
    -- 设置定期任务，每隔 TAUNT_PERIOD 时间执行一次 TauntCreatures 函数
    inst._taunttask = inst:DoPeriodicTask(TAUNT_PERIOD, TauntCreatures, 0)
    
    -- 设置加载时执行的函数为 OnLoad
    inst.OnLoad = OnLoad
    -- 设置进入非活动状态时执行的函数为 goinactive
    inst.GoInactive = goinactive
    -- 设置对象进入睡眠状态时执行的函数为 OnEntitySleep
    inst.OnEntitySleep = OnEntitySleep
    -- 设置对象醒来时执行的函数为 OnEntityWake
    inst.OnEntityWake = OnEntityWake
    -- 设置领袖变化时执行的函数为 onLeaderChanged
    inst.onLeaderChanged = onLeaderChanged
    -- 设置判断领袖是否疯狂的函数为 commonfn.isleadercrazy
    inst.isleadercrazy = commonfn.isleadercrazy
    -- 设置检查联盟的函数为 CheckForAllegiances
    inst.CheckForAllegiances = CheckForAllegiances
    -- 设置暴躁的函数为 commonfn.hotheaded
    inst.hotheaded = commonfn.hotheaded
    
    -- 设置监听事件，当对象被攻击时执行 OnAttacked 函数
...
return Prefab("cp_bernie_big", fn, assets, prefabs),
