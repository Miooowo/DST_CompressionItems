local assets =
{
    Asset("ANIM", "anim/cp_pocketwatch_weapon.zip"),
	Asset("ATLAS", "images/cp_pocketwatch_weapon.xml"),  -- XML文件
	Asset("IMAGE", "images/cp_pocketwatch_weapon.tex")
}

local prefabs = 
{
	"cp_pocketwatch_weapon_fx",
}
...
local function TryStartFx(inst, owner)
    ...
    if not inst.components.fueled:IsEmpty() then
        ...
        if inst._vfx_fx_inst == nil then
            inst._vfx_fx_inst = SpawnPrefab("cp_pocketwatch_weapon_fx")
            if inst._vfx_fx_inst == nil then
                print("Error: Could not spawn VFX prefab")
                return
            end
...
local function onequip(inst, owner)
    owner.AnimState:OverrideSymbol("swap_object", "cp_pocketwatch_weapon", "swap_object")
    ...
    if data and data.percent then
        if data.percent > 0 then
            if not inst:HasTag("shadow_item") then
			    ...
			    inst.components.weapon:SetDamage(BaseDamge)
				TryStartFx(inst)
            end
        else
            inst:RemoveTag("shadow_item")
		    inst.components.weapon:SetDamage(DepletedDamge)
			StopFx(inst)
        end
    end
end
...
local function fn()
    ...
    inst.AnimState:SetBank("cp_pocketwatch_weapon")
    inst.AnimState:SetBuild("cp_pocketwatch_weapon")
    ...
    inst.scrapbook_fueled_rate = TUNING.TINY_FUEL * 40
	...
	inst.components.inventoryitem.imagename = "cp_pocketwatch_weapon"
	inst.components.inventoryitem.atlasname = "images/cp_pocketwatch_weapon.xml"
    ...
    inst.components.weapon:SetDamage(BaseDamge)
    inst.components.weapon:SetRange(TUNING.WHIP_RANGE*40)
    inst.components.weapon:SetOnAttack(onattack)

    inst:AddComponent("fueled")
    inst.components.fueled.fueltype = FUELTYPE.NIGHTMARE
    inst.components.fueled:InitializeFuelLevel(4 * TUNING.LARGE_FUEL*40)
...
return Prefab("cp_pocketwatch_weapon", fn, assets, prefabs)
